...
### 6. プランごとに受付を止める／始める（OFFLINEの受付開始前など）

`index.html` 冒頭の `PLAN_ON_SALE` で、プランごとに申し込みの受付を切り替えられます。

```js
const PLAN_ON_SALE = {
  online:  true,
  offline: false    // OFFLINEは受付開始まで false のまま
};
```

- `false` のプランは、PRICEページ・JOINページのボタンが「受付準備中」になり押せなくなります（Stripeの決済ページには飛びません）。
  カードには「OFFLINEプランの受付は準備中です…」の案内が出ます。
- 受付を始めるときは `true` に変えて push するだけです。`STRIPE_LINKS` の支払いリンクは貼ったままで構いません。
- Stripe側で支払いリンクを「無効」にしておく必要はありませんが、念のため無効にしておくとURLを直接知っている人からの申し込みも防げます。
